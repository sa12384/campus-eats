        ...
        const order = await t.one(       
            'INSERT INTO orders (menu_item_id, quantity, price_at_order) VALUES ($1, 1, $2) RETURNING *',      
             ...
             await t.none('UPDATE menu_items SET times_ordered = times_ordered + 1 WHERE id = $1', [menuItemId]);    
              return order;   }); };  exports.getOrderById = (id) => {   return db.oneOrNone(     `SELECT orders.*, menu_items.name AS item_name      FROM orders      JOIN menu_items ON orders.menu_item_id = menu_items.id      WHERE orders.id = $1`,     [id]   ); }; 
            ...
            exports.cancelOrder = (id) => {   
                ...
                exports.getStats = () => {   return db.one(`     
                    SELECT COUNT(*) AS total_orders,            
                    COALESCE(SUM(price_at_order * quantity), 0) AS total_revenue     FROM orders   `); };  
